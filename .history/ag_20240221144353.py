...
def selection_operator(survivor_percentage, poblacion):
    ...
    scores = np.empty(len(poblacion))

    for i in range(len(poblacion)):
        scores[i] = fitness_func(poblacion[i])
    ...
    seleccionados = np.empty(num_sobrevivientes, len(poblacion[9]))

    for i in range(num_sobrevivientes):
        seleccionados[i] = sorted_individuals[i]
