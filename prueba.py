...
def fitness_func(cromosoma, value_vector=VALUE_VECTOR):
  crom = np.array(cromosoma, dtype=int)
  values = np.array(value_vector)
  valor_total = 0
  for i in range(len(crom)):
    objeto_elegido = int(crom[i])
    valor_objeto = int(values[i])
    if objeto_elegido==1:
...
def one_point_crossover(elite, tam_pob):
  ...
  random_point = np.random.randint(0, len(elite[0]))
...
def bit_flip_mutation(poblacion,porcentaje_mutacion=PORCENTAJE_MUTACION, bits=BITS_MUTACION):
  ...
  for cromosoma in poblacion:
    ...
    if loteria == 50:
      total_mutados+=1
      for _ in range(bits):
        random_bit = np.random.randint(0, len(cromosoma))
